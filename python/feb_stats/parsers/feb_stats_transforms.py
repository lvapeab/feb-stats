...
def transform_cum_stats_minutes(minutes_timeseries: pd.Series,
                                prefix='minutes') -> pd.DataFrame:
    """Shots with the format made-attempted"""
    cast_fn = lambda x: '00:' + str(x)  # Add hours to min:seconds
    return pd.DataFrame({prefix: minutes_timeseries.map(cast_fn)})


def transform_cum_stats_blocks(blocks_series: pd.Series,
                               prefix='blocks') -> pd.DataFrame:
    """Blocks with the format made/received"""

    return pd.DataFrame(blocks_series.str.split(' ').tolist(),
                        columns=[f'{prefix}_made', f'{prefix}_received'],
                        dtype='float32')


def transform_cum_stats_fouls(fouls_series: pd.Series,
                              prefix='fouls') -> pd.DataFrame:
    """Fouls with the format made/received"""
    return pd.DataFrame(fouls_series.str.split(' ').tolist(),
                        columns=[f'{prefix}_made', f'{prefix}_received'],
                        dtype='float32')


def transform_cum_stats_rebounds(rebs_series: pd.Series,
                                 prefix='rebounds') -> pd.DataFrame:
    """Fouls with the format made/received"""
    return pd.DataFrame(rebs_series.str.split(' ').tolist(),
                        columns=[f'defensive_{prefix}',
                                 f'offensive_{prefix}',
                                 f'total_{prefix}',
                                 ],
                        dtype='float32')
...
def transform_game_stats_df(initial_df: pd.DataFrame,
    ...
    transform_keys = {
        ...
        'T.Camp': ('field_goals', transform_cum_stats_shots),
        'T.L': ('free_throws', transform_cum_stats_shots),
    ...
    for transform_key, transform_tuple in transform_keys.items():
        ...
        new_df = transform_function(df.loc[:, transform_key],
                                    prefix=new_name)
        df = pd.concat([df, new_df], axis=1)
        df = df.drop(axis='columns',
                     labels=transform_key)

    df.at[df.shape[0] - 1, 'point_balance'] = 0.
