...
class FEBParser(GenericParser):
    ...
    def parse_game_stats(self,
        ...
        home_team = Team(
            id=int(md5(str.encode(metadata['home_team'], encoding='UTF-8')).hexdigest(), 16),
            name=metadata['home_team']
        )
        away_team = Team(
            id=int(md5(str.encode(metadata['away_team'], encoding='UTF-8')).hexdigest(), 16),
        ...
        game = Game(
            ...
            main_referee=Player(
                id=int(md5(str.encode(metadata['main_referee'], encoding='UTF-8')).hexdigest(), 16),
                name=metadata['main_referee']
            ),
            aux_referee=Player(
                id=int(md5(str.encode(metadata['second_referee'], encoding='UTF-8')).hexdigest(), 16),
                name=metadata['second_referee']
            ),
            local_boxscore=Boxscore(
                id=int(md5(str.encode(f"{metadata['league']}_{metadata['date']}_{metadata['home_team']}",
                                      encoding='UTF-8')).hexdigest(), 16),
                boxscore=game_stats['home_boxscore']
            ),
            away_boxscore=Boxscore(
                id=int(md5(str.encode(f"{metadata['league']}_{metadata['date']}_{metadata['away_team']}",
                                      encoding='UTF-8')).hexdigest(), 16),
