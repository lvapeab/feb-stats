...
from python.feb_stats.locale import spanish_columns


def league_to_xlsx(league: League,
                   filename: str = None,
                   col_width: int = 30,
                   export_language: str = 'es') -> bytes:
    ...
    xlsx_writer = pd.ExcelWriter(filename,
                                 engine='openpyxl',
                                 mode='w',
    ...
    if league.aggregated_games is not None:
        ...
        column_names = map(lambda x: spanish_columns[x], columns) if export_language == 'es' else columns
        ...
        player_column_names = map(lambda x: spanish_columns[x],
                                  player_columns) if export_language == 'es' else player_columns
